...
HRESULT golem::init(int x, int y)
{
	_move = new animation;
	...
	_move->init(IMAGEMANAGER->findImage("골렘전사"), 0, 6, true);
	_attack->init(IMAGEMANAGER->findImage("골렘공격"), 0, 6);
...
void golem::render()
{
	...
	case EM_ATTACK:
		...
		break;
	case EM_DIE:
		_die->CameraRender(getMemDC(), _x - 60, _y - 95);
		break;
	}
