...
HRESULT image::initForStretch(int sizeX, int sizeY)
{
	HDC hdc = GetDC(_hWnd);
...
void image::stretchRender(HDC hdc, int destX, int destY, int sizeX, int sizeY)
{
	...
	if (_isTrans)
	{
		...
		StretchBlt(_stretchImage->hMemDC, 0, 0, sizeX, sizeY, _imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, SRCCOPY);
		GdiTransparentBlt(hdc, destX, destY, sizeX, sizeY, _stretchImage->hMemDC, 0, 0, sizeX, sizeY, _transColor);
	}
	else
	{
		StretchBlt(hdc, destX, destY, sizeX, sizeY, _imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, SRCCOPY);
	}
}

void image::stretchRender(HDC hdc, int destX, int destY, float size)
{
	int sizeX = _imageInfo->width * size;
	int sizeY = _imageInfo->height * size;
	this->initForStretch(sizeX, sizeY);
	SetStretchBltMode(hdc, COLORONCOLOR);
	SetStretchBltMode(_stretchImage->hMemDC, COLORONCOLOR);
	if (_isTrans)
...
void image::stretchFrameRender(HDC hdc, int destX, int destY,int currentFrameX, int currentFrameY, int sizeX, int sizeY)
{
	...
	{
		StretchBlt(hdc, destX, destY, sizeX, sizeY, _imageInfo->hMemDC, 0, 0, _imageInfo->frameWidth, _imageInfo->frameHeight, SRCCOPY);
	}
}

void image::stretchFrameRender(HDC hdc, int destX, int destY, int currentFrameX, int currentFrameY, float size)
{
	int sizeX = _imageInfo->frameWidth * size;
	int sizeY = _imageInfo->frameHeight * size;
